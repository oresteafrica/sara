...
if (check_get('n')) {
	...
	$sql = 'SELECT id, (SELECT name FROM officer WHERE officer.id = from_field_1.op_id) as Usuário, DATE_FORMAT(in_date, "%d/%m/%Y") as Data, ( SELECT name FROM areas WHERE areas.id = from_field_1.MZ006) as Província, CASE WHEN from_field_1.MZ007_n = 0 THEN MZ007 ELSE ( SELECT name FROM areas WHERE areas.id = from_field_1.MZ007_n ) END as Distrito, CASE WHEN from_field_1.MZ003_n = 0 THEN MZ003 ELSE ( SELECT name FROM units WHERE units.id = from_field_1.MZ003_n ) END as "Nome da US" FROM from_field_1 ORDER BY in_date';
